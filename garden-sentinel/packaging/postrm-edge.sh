#!/bin/bash
#
# Post-removal script for garden-sentinel-edge
#

# Only run on purge (not upgrade)
if [ "$1" = "purge" ] || [ "$1" = "0" ]; then
    echo "Cleaning up Garden Sentinel Edge..."
    ...
    rm -f /etc/garden-sentinel/edge.yaml

    # Remove user if no other garden-sentinel packages installed
    if ! dpkg -l | grep -q "garden-sentinel-server"; then
